...
//		HashSet(집합)은 중복되는 데이터 입력을 허용하지 않는다.
//		데이터를 입력하는 순서와 입력된 데이터가 실제로 저장되는 순서가 달라서 get(), set()과 같이
//		인덱스를 지정하는 메소드가 제공되지 않는다.
...
//			그렇지 않으면 false를 리턴한다.
//			if (hset.contains(bonus) == false) { // 보너스 번호가 1등 번호에 포함되지 않았는가?
