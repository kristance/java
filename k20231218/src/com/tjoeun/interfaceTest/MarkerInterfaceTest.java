...
//	상수를 초기화시키는 방법은 3가지가 있는데 첫 번째 방법은 선언시 "="을 사용해서
//	초기화시키는 방법이고 두 번째 방법은 객체가 생성될 때 생성자에서 초기화시키는
//	방법이다.
//	세 번째 방법은 static으로 선언한 상수를 초기화하는데 2문장 이상 필요한 경에는
//	static { }와 같이 초기화 블록을 사용하는 방법이 있다.
	public final int MAX_HP; // 최대 HP, 상수
	int currentHP; // 현재 HP
	
//	생성자를 선언하지 않으면 자바 컴파일러가 아무런 일도 하지 않는 기본 생성자를 만들어
//	주지만 기본 생성자를 선언하지 않고 다른 생성자를 선언하면 자바 컴파일러는 기본 생성자를
//	자동으로 만들어주지 않는다.
...
//		생성자 GroundUnit(int hp)를 호출한다.
		super(150);
//		⑦ Tank 클래스의 MAX_HP가 150으로 초기화되었으므로 현재 HP(currentHP)를 MAX_HP로
//		초기화시킨다.
		currentHP = MAX_HP;
		System.out.println("Tank의 MAX_HP는 " + MAX_HP + "입니다.");
...
class Marine extends GroundUnit {
	...
	public Marine() {
		super(80);
		currentHP = MAX_HP;
		System.out.println("Marine의 MAX_HP는 " + MAX_HP + "입니다.");
...
class SCV extends GroundUnit implements Repairable {
	...
	public SCV() {
		super(100);
		currentHP = MAX_HP;
		System.out.println("SCV의 MAX_HP는 " + MAX_HP + "입니다.");
		System.out.println("SCV의 현재 HP는 " + currentHP + "입니다.");
	}
	
//	수리할 객체를 넘겨받아 수리하는 메소드를 만든다.
...
//		표시 인터페이스 타입으로 수리할 객체를 받았는데 표시 인터페이스에는 아무런 내용이
//		코딩되어있지 않기 때문에 인수로 넘겨받은 객체를 실제로 현할 내용이 들어있는 
//		클래스로 형변환 시킨 후 처리해야 한다.
		
//		if (repairable instanceof Tank) {
//			Tank tank = (Tank) repairable;
//		} else if (repairable instanceof SCV) {
//			SCV scv = (SCV) repairable;
//		} else if (repairable instanceof DropShip) {
//			DropShip dropShip = (DropShip) repairable;
//		}
...
//			수리한다.
			...
			System.out.println("Repairable 인터페이스로 넘어온 객체를 Unit 클래스로 형변환 할 수 없습니다.");
		}
...
class DropShip extends AirUnit implements Repairable {
	...
	public DropShip() {
		super(120);
		currentHP = MAX_HP;
		System.out.println("DropShip의 MAX_HP는 " + MAX_HP + "입니다.");
